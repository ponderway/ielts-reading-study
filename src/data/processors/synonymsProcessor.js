...
async function processSynonyms() {
    try {
        ...
        // 读取 vocabulary.md 文件
        const vocabularyPath = path_1.default.join(__dirname, '../../../ielts-materials/vocabulary.md');
        const content = fs_1.default.readFileSync(vocabularyPath, 'utf8');
        // 解析同义词数据
        ...
        // 处理 List_1.md 文件
        const list1Path = path_1.default.join(__dirname, '../../../ielts-materials/assets/Vocabulary/List_1.md');
        if (fs_1.default.existsSync(list1Path)) {
            const list1Content = fs_1.default.readFileSync(list1Path, 'utf8');
        ...
        for (const [word, syns] of synonymMap.entries()) {
            try {
                // 查找单词 ID
                const wordRecord = await database_1.db.get(`SELECT id FROM words WHERE word = ?`, [word]);
                if (wordRecord) {
                    const wordId = wordRecord.id;
                    // 插入同义词
                    for (const synonym of syns) {
                        await database_1.db.run(`INSERT OR IGNORE INTO synonyms (word_id, synonym, created_at)
               VALUES (?, ?, CURRENT_TIMESTAMP)`, [wordId, synonym]);
                    }
...
exports.default = processSynonyms;
